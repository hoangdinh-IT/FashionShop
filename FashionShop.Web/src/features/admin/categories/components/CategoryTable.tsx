...
const CategoryTable: React.FC<Props> = ({ data, isLoading, sortBy, isAscending, onSort, onEdit, onDelete }) => {
    ...
    const SortableHeader = ({ label, colKey, align = 'left' }: { label: string, colKey: string, align?: 'left' | 'center' | 'right' }) => {
        ...
        return (
            <div 
                className={`flex items-center gap-1 cursor-pointer group select-none text-[13px] ${alignClass}`}
                onClick={() => handleHeaderClick(colKey)}
            >
                <span>{label}</span>
                <span className="text-gray-400 group-hover:text-indigo-600 transition-colors">
                    {isActive ? (
                        isAscending ? <IoArrowUp size={13} className="text-indigo-600" /> : <IoArrowDown size={13} className="text-indigo-600" />
                    ) : (
                        <IoSwapVertical size={13} className="opacity-0 group-hover:opacity-50" />
                    )}
                </span>
            </div>
        );
    ...
    const TableRowSkeleton = () => (
        <tr className="animate-pulse border-b border-gray-50">
            ...
            <td className="px-5 py-4"><div className="h-4 bg-gray-200 rounded w-8 mx-auto"></div></td>
            <td className="px-5 py-4"><div className="h-6 bg-gray-200 rounded-full w-20 mx-auto"></div></td>
                                ...
                                <tr key={item.id} className="group hover:bg-indigo-50/40 transition-colors duration-200">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-lg shrink-0 border border-gray-200 shadow-sm group-hover:shadow-md group-hover:border-indigo-100 transition-all duration-300 overflow-hidden bg-white flex items-center justify-center p-0.5">
                                                {item.imageUrl ? (
                                                    <img
                                                        src={item.imageUrl}
                                                        alt={item.name}
                                            ...
                                            <div className="flex flex-col gap-0.5">
                                                <div className="flex items-center gap-1.5">
                                                    <div className="font-semibold text-gray-700 text-[14px] group-hover:text-indigo-600 transition-colors">
                                                        {item.name}
                                                    </div>
                                                    ...
                                                    {item.description && (
                                                        <Tooltip 
                                                            ...
                                                            content={
                                                                <div className="whitespace-pre-line text-left max-w-xs text-sm">
                                                                    {item.description}
                                                                </div>
                                                            }
                                                        >
                                                            <div className="cursor-help opacity-0 group-hover:opacity-100 transition-opacity">
                                                                <IoInformationCircleOutline className="text-gray-400 hover:text-indigo-500 text-sm" />
                                                            </div>
                                                        </Tooltip>
                                                    )}
                                                </div>
                                    ...
                                    <td className="px-5 py-4">
                                        <span className="inline-block max-w-[135px] truncate px-2 py-1 bg-gray-50 rounded-md text-[10px] font-mono font-medium text-gray-500 border border-gray-100 group-hover:border-indigo-100 group-hover:bg-white group-hover:text-indigo-500 transition-all select-all">
                                            /{item.slug}
                                        </span>
                                    </td>
