...
const SizeTable: React.FC<Props> = ({
    ...
    const getSizeTypeLabel = (type: SizeType | string) => {
        switch(type) {
            case SizeType.Clothing:
            case "Clothing":
                return { label: 'Quần áo', color: 'text-blue-600 bg-blue-50 border-blue-100' };
            case SizeType.Footwear: 
            case "Footwear":
                return { label: 'Giày dép', color: 'text-orange-600 bg-orange-50 border-orange-100' };
            default: return { label: 'Phụ kiện', color: 'text-purple-600 bg-purple-50 border-purple-100' };
        }
    };
    ...
    return (
        <div className="bg-white rounded-[20px] shadow-[0_0_32px_-10px_rgba(0,0,0,0.05)] border border-gray-100/80 overflow-hidden flex flex-col font-sans">
                    ...
                    <tbody className="divide-y divide-gray-50/80">
                        {isLoading ? (
                            ...
                            data.map((item) => {
                                ...
                                const typeInfo = getSizeTypeLabel(item.type);
                                    ...
                                    <tr key={item.id} className="group hover:bg-indigo-50/40 transition-colors duration-200">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-lg shrink-0 border border-gray-200 bg-gray-50 flex items-center justify-center shadow-sm group-hover:shadow-md group-hover:border-indigo-200 group-hover:bg-white transition-all duration-300">
                                                    <span className="font-bold text-gray-600 text-base group-hover:text-indigo-600 transition-colors">
                                                        {item.name}
                                                    </span>
                                                </div>
                                        ...
                                        <td className="px-5 py-4">
                                            <span className={`inline-block px-2 py-0.5 rounded-md text-[10px] font-medium border ${typeInfo.color} transition-all`}>
                                                {typeInfo.label}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-5 py-4 text-center">
                                            <div className="flex items-center justify-center gap-1 text-gray-500">
                                                <IoLayersOutline className="text-gray-400 text-xs" />
                                                <span className="font-mono font-semibold text-xs">{item.sortOrder}</span>
                                            </div>
                                        </td>
                                        ...
                                        {/* 6. TRẠNG THÁI */}
                                        <td className="px-5 py-4 text-center">
                                            <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold border shadow-sm transition-transform hover:scale-105 ${
                                                item.isActive 
