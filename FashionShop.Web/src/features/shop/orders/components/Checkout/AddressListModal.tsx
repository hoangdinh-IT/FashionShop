...
import { X, MapPin, Phone, Check, Plus } from 'lucide-react';
import type { Address } from '../../../addresses/types/address';
import AddressString from '../../../addresses/components/AddressString';
import AddressFormModal from '../../../addresses/components/AddressFormModal'; // Thêm import này (điều chỉnh path phù hợp)
...
const AddressListModal: React.FC<Props> = ({
    isOpen,
    onClose,
    addresses,
    onSelect,
    ...
    // State quản lý ẩn/hiện AddressFormModal
                ...
                {isOpen && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 md:p-6">
                        ...
                        >
                            ...
                            <div className="px-4 sm:px-6 md:px-7 pt-3.5 sm:pt-4 shrink-0">
                                <button
                                    type="button"
                                    onClick={() => setIsFormOpen(true)}
                                    className="w-full py-2.5 sm:py-3 border border-dashed border-zinc-300 hover:border-zinc-900 hover:bg-zinc-50 rounded-xl sm:rounded-2xl text-xs sm:text-sm font-semibold text-zinc-700 hover:text-zinc-900 flex items-center justify-center gap-2 transition-all cursor-pointer active:scale-[0.99]"
                                >
                                    <Plus size={16} />
                            ...
                            {/* Body - Address List */}
                            <div className="px-4 sm:px-6 md:px-7 py-3 sm:py-4 overflow-y-auto flex-1 space-y-2.5 sm:space-y-3 max-h-[50vh] sm:max-h-[55vh]">
                                {addresses.length === 0 ? (
